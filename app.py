...
def get_db():
    if not firebase_admin._apps:
        if os.path.exists("firebase-key.json"):
            ...
            st.error("Authentication Failure: System cannot locate Firebase credentials.", icon=":material/lock:")
            st.stop()
...
def load_live_data():
    try:
        ...
        st.error(f"Telemetry Error: {e}", icon=":material/error:")
        return pd.DataFrame()
...
with c1:
    st.title(":material/monitoring: FlowTrack Enterprise")
...
with c2:
    if not df.empty:
        ...
        st.success(f"**SECURE UPLINK ACTIVE** | T= {last_sync}", icon=":material/cell_tower:")
    else:
        st.warning("**SYSTEM IDLE** | Awaiting Telemetry", icon=":material/sync_disabled:")
        st.stop()
...
with col_ai:
    st.subheader(":material/memory: AI Dispatch Oracle")
    ...
    with st.container(border=True):
        ...
        if gemini_key:
            ...
            if st.button("Request Tactical Briefing", use_container_width=True, icon=":material/graphic_eq:"):
                with st.spinner("Analyzing telemetry..."):
                    try:
                        ...
                        st.info(response.text, icon=":material/tips_and_updates:")
                    except Exception as e:
                        st.error(f"API Error: {e}", icon=":material/gpp_bad:")
        else:
            st.info("System requires Gemini API key to generate briefings.", icon=":material/key:")

with col_data:
    st.subheader(":material/table_chart: Live Corridor Telemetry")
