...
<style>
    ...
    /* Status Pill Badges for Map Popups */
...
# --- 7. 4D SPATIAL MAP (LINES & MARKERS) ---
st.markdown("<h3 style='color: #E0E0E0; font-size: 1.2rem; margin-top: 20px; margin-bottom: 5px;'>:material/map: Geospatial Telemetry Grid</h3>", unsafe_allow_html=True)

# Start and End coordinates for drawing actual roads
road_segments = {
...
# Center on Dar es Salaam, zoom tighter so roads are visible immediately
...
for idx, row in df.iterrows():
    if row["road_id"] in road_segments:
        ...
        if row["status"] == "Heavy Jam" or row["delay_mins"] > 15:
            hex_color, folium_color, badge_class = "#FF4B4B", "red", "badge-danger"
            status_text, map_weight = "CRITICAL GRIDLOCK", 6
        elif row["status"] == "Moderate" or row["delay_mins"] > 5:
            hex_color, folium_color, badge_class = "#F6C85F", "orange", "badge-warn"
            status_text, map_weight = "MODERATE FRICTION", 4
        else:
            hex_color, folium_color, badge_class = "#00CC96", "green", "badge-clear"
            status_text, map_weight = "OPTIMAL FLOW", 3

        # HTML Popup (Dark Mode)
        popup_html = f"""
        ...
        # 2. DRAW THE CENTER DOT (No clustering!)
        folium.CircleMarker(
